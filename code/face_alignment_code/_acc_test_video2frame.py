...
def video2frame(video_input, frame_output):
    ...
    while(True):
        ...
        if ret:
            idx += 1
            cv2.imwrite( frame_output+'/'+ ( '%08d' % idx) + '.jpg' ,frame)
        else:
            break
    cap.release()
...
if __name__ == '__main__':
    video_dir_train ='/data4/AFEW/train/'
    frame_dir_train = '/data4/AFEW/train_frame/'
    
    video_dir_val ='/data4/test_data_for_emotion-fan/test2_video/'
    frame_dir_val = '/data4/test_data_for_emotion-fan/test2_frame/'
    
    # main(video_dir_train, frame_dir_train, n_thread =20)
    main(video_dir_val, frame_dir_val, n_thread = 1)