...
def proc_frames(src_path, dst_path):
    cmd = 'ffmpeg -i \"{}\" -start_number 0 -qscale:v 2 -vf scale=224:224 \"{}\"/%06d.jpg -loglevel error -y'.format(src_path, dst_path)
    os.system(cmd)
...
if __name__ == "__main__":
    ...
    if args.src_input_path.split('/')[-1].split('.')[-1] == 'mp4':
        ...
        # os.system('ffmpeg -i {} -vf scale=224:224 {}'.format( args.src_input_path, os.path.join(dst_input_path, args.src_input_path.split('/')[-1]) ))
    ...
    if args.src_audio_path.split('/')[-1].split('.')[-1] == 'mp3':
        ...
        if args.src_mouth_frame_path and os.path.isdir(args.src_mouth_frame_path):
        ...
        elif args.src_mouth_frame_path:
            print('***')
            ...
            num_mouth_frames = proc_frames(args.src_mouth_frame_path, dst_mouth_frame_path)
    ...
    csv_dir = os.path.split(args.csv_path)[0]
    mkdir(csv_dir)
