...
def align_folder(folder_path, folder_save_path = '', fa = None ):

    if folder_save_path == '':
        home_path = '/'.join(folder_path.split('/')[:-1])
        folder_save_path = os.path.join(home_path, folder_path.split('/')[-1] + '_cropped')
    
    os.makedirs(folder_save_path,exist_ok=True)

    if fa is None : fa = face_alignment.FaceAlignment(face_alignment.LandmarksType._2D, device=device)
    preds = fa.get_landmarks_from_directory(folder_path)
    # print('1111',len(preds),'---')
    ...
    for img in preds.keys():
        ...
        if pred_points is None or len(pred_points.shape) != 3:
            ...
            if num_faces >= 1 and size == 68:
                ...
                print(num_faces,size)
                print('preprocessing failed')
                return False
    if len(preds) == 0: 
        print('number of preds is zero!')
        return False
    ...
    for i, img_pth in enumerate(preds.keys()):
        ...
        # folder_save_path = '/data4/talking_head_testing/25fps_video/no_crop_frames/test_dir/aaa'
        img_save_path = os.path.join(folder_save_path, img_pth.split('/')[-1])
        cv2.imwrite(img_save_path, wrapped)
        # print(img_save_path , wrapped.shape  )
...
def main():
    ...
    if os.path.isdir(args.folder_path):
        ...
        # if os.path.exists( save_img_path ) : os.system('rm -drf {}'.format(save_img_path))
...
# CUDA_VISIBLE_DEVICES=2 python scripts/align_68.py --folder_path /data4/talking_head_testing/temp_files_when_gening/pc_avs_temp_files/temp_0722/Pose_Source/M003_ang_3_001
# CUDA_VISIBLE_DEVICES=2 python scripts/align_68.py --folder_path /data4/talking_head_testing/temp_files_when_gening/pc_avs_temp_files/temp_0722/Input/M003
